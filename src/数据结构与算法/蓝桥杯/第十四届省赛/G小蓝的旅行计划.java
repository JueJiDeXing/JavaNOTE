...
public class G小蓝的旅行计划 {
    ...
    private static void main_enter1() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int maxAdd = Math.min(m - curr, sc.nextInt());// 最多加的油量(最多加满)
...
class Main {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            while (curr < 0) {//走不到下一站,需要加-curr升油
                ...
                if (cnt <= -curr) {//不够,cnt升全部都加
                    ...
                    curr += cnt;
                    limit[a.id] = 0;
                    tree.add(1, 1, n, a.id, i - 1, cnt);//对[a.id,i-1]内的站点加cnt
                } else {// cnt升加完还会有剩余
                    ans += a.cost * (-curr);
    ...
    static class SegmentTree {
        ...
        //与传统线段树不同点在于数据rest的运算不通过Add,而是取Max
        //无build函数,因为rest数据初始全为0,到达站点后才会进行更新
