...
public class _2258逃离火灾 {
    ...
    boolean check(int t) {
        ...
        while (t-- > 0) update(fire, true, 0);  // 先执行 t 秒的火势蔓延,时间偏移offset=0表示归类于初始火源
    ...
    void update(Deque<int[]> deque, boolean isFire, int offset) {//offset:时间偏移量,用于安全屋处的判断
        int sz = deque.size();
        while (sz-- > 0) {
            ...
            for (int[] dir : dirs) {//搜索四个方向
                ...
                if (g[nx][ny] == 2) continue;//是墙
                if (isFire) {
                    //火
                    if (fg[nx][ny] != 0) continue;//这个地方不是草地,无法蔓延
                    fg[nx][ny] = fg[x][y] + offset;//蔓延,fg存储火蔓延到此次的时间
                } else {
                    //人
                    if (nx == n - 1 && ny == m - 1 && (fg[nx][ny] == 0 || fg[nx][ny] == pg[x][y] + offset)) {
                        ok = true;  // 人到达安全屋 并且 (火尚未到达 或 人火同时到达) -> 因为火是先更新的,所以人加offset的1秒到达的话也是成立的
                    }
                    if (fg[nx][ny] != 0 || pg[nx][ny] != 0) continue;//不是草地 -> 是 有火的地方 或 人已经走过了
                    pg[nx][ny] = pg[x][y] + offset;//pg存储的是走的时间,每次加1秒
                }
...
/**
 ...
 <li>case 3: f < p (都连通,但是火比人快), 返回-1</li>
 <li>case 4: f>=p (都连通,人比火快)<br>
