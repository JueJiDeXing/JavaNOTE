...
public class 盒子与球 {
    ...
    返回一个长度为 n 的数组 answer,其中answer[i]是将所有小球移动到第 i 个盒子所需的 最小生成树 操作数。
    ...
    private static int[] move(String boxes) {
        ...
        //定义变量记录左右两侧的小球数量,以及小球左右两侧的操作数
        if (boxes.charAt(0) == '1') {
            right_num--;
        }
        int left_count = 0;//左、右操作数,初始左操作数为0,右操作数为移动到索引0的盒子的总操作数
        int right_count = res[0];
        /* 公式
