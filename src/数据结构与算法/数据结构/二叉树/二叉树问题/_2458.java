...
public class _2458 {
    ...
    /**
     对于节点node,假设删除它的剩余高度为H(node),它的深度为D(node)
     那么删除它的左子树剩余高度为 不含node的其他子树的高度 与 右子树高度 的较大值
     即 H(left)=max( H(node), D(node)+Height(node.right) )    right同理
     其中每个节点的Height可以通过一次dfs求出
     每个节点的D与答案可以在第二次dfs时递推
     */
